...
namespace d3dUtils
{
	...
	ID3D11Texture2D* CreateTexture(uint32 width, uint32 height, UINT bindFlags, DXGI_FORMAT format, void* data, size_t size);
	...
	ID3DBlob* CompileShader(const char* target, void* src, size_t srcLength);
	...
	ID3D11Buffer* CreateBuffer(UINT bindFlags, UINT cpuAccessFlags, D3D11_USAGE usage, size_t size, void* initialData);
	void OverwriteBuffer(ID3D11Buffer* buffer, void* data, size_t size);
}
